module vault::dgt {
    ...
    fun init(witness: DGT, ctx: &mut TxContext) {
        let (treasury_cap, metadata)
            = coin::create_currency<DGT>(
            witness,
            2, // decimals
            b"DGT", // symbol
            b"Digitrust", // name
            b"digitrust asset", // description
            option::none(),
            ctx
        );
    ...
    public entry fun mint(
        treasury_cap: &mut TreasuryCap<DGT>, amount:u64, recipient: address, ctx: &mut TxContext
    ) {
        coin::mint_and_transfer(treasury_cap, amount, recipient, ctx)
    }

    public entry fun burn(treasury_cap: &mut TreasuryCap<DGT>, coin: Coin<DGT>) {
    ...
    public fun test_mint() {
        ...
        {
            let treasuryCap = test_scenario::take_from_sender<TreasuryCap<DGT>>(scenario);
        ...
        {
            let mycoin = test_scenario::take_from_sender<Coin<DGT>>(scenario);
